...
export const useAuthStore = defineStore('authStore', () => {
  ...
  const login = async (email: string, password: string, isSuperAdmin = false): Promise<ApiResponse<IAuthResponce>> => {
    try {
      ...
      const url = isSuperAdmin ? staticEndpoints.auth.loginAdmin : staticEndpoints.auth.loginUser;

      const response: ApiResponse<IAuthResponce> = await axiosInstance.post(url, { email, password });
  ...
  const register = async (
    ...
    try {
      loading.value = true;
      const url = isSuperAdmin ? staticEndpoints.auth.registerAdmin : staticEndpoints.auth.registerUser;
