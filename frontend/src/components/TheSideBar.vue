<script setup lang="ts">
  ...
  const logOutUser = (): void => {
    ...
    toast.add({
      severity: 'success',
      summary: 'User successfully logout',
      life: 3000,
    });
    setTimeout(() => {
    ...
    <ul class="m-0 w-full list-none overflow-hidden p-0">
      <li>
        <router-link
          ...
          to="/"
          :class="[
            'flex items-center justify-center rounded p-4 transition-colors duration-150',
            isActiveRoute('/')
      ...
      <li>
        <router-link
          ...
          :class="[
            'flex items-center justify-center rounded p-4 transition-colors duration-150',
            isActiveRoute('/goods-receipt')
              ? 'text-primary-500 font-semibold'
              : 'text-surface-700 hover:bg-surface-200 dark:text-surface-0 dark:hover:bg-surface-800',
          ]"
        >
          <i class="pi pi-angle-double-up mr-2"></i>
          <span class="font-medium">Goods Receipt</span>
        </router-link>
      </li>
      <li>
        <router-link
          ...
          :class="[
            'flex items-center justify-center rounded p-4 transition-colors duration-150',
            isActiveRoute('/stock-activity')
              ? 'text-primary-500 font-semibold'
              : 'text-surface-700 hover:bg-surface-200 dark:text-surface-0 dark:hover:bg-surface-800',
          ]"
        >
          <i class="pi pi-angle-double-down mr-2"></i>
          <span class="font-medium">Stock Activity</span>
        </router-link>
      </li>
      <li>
        <router-link
