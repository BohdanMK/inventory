<script setup lang="ts">
  ...
  const createStatus = async (value: string): Promise<void> => {
    try {
      const { success, message, data } = await statusStore.createStatus(value);
  ...
  const deleteItem = async (id: string | number): Promise<void> => {
    try {
      const { success, message, data } = await statusStore.deleteStatus(id);
      if (success) {
        toast.add({ severity: 'success', detail: message, life: 3000 });
...
<template>
  <div>
    ...
    <div v-else class="card">
      ...
      <Toast />
      <Toolbar class="mb-6">
        <template #start><span class="text-xl font-bold">Statuses</span></template>
        <template #end>
      ...
      <div v-if="!asyncState.loadingStatus.value">
        <DataTable
          :value="statusStore.statusesList"
