...
<template>
  <div class="absolute bottom-[-10px] left-[28px] flex cursor-pointer">
    <div v-for="(reaction, index) in message.reactions" :key="reaction.userId" @click="toggle">
      <div v-if="index < 2" :class="index > 0 ? 'ms-[-12px]' : ''">
        {{ reaction.emoji }}
      </div>
    </div>
    <div v-if="visibleStatus" class="rounded-sm bg-white p-0 pe-1">+{{ reactionAdditionalLength }}</div>
    <Popover ref="op" class="p-3">
      <div class="flex flex-col gap-2">
        <div v-for="reaction in message.reactions" :key="reaction.userId" class="flex items-center gap-2 py-1 pe-2">
          <span>{{ reaction.emoji }}</span>
          <div
            v-if="userProfile.userProfile.username !== reaction.username"
            class="flex w-full items-center justify-between gap-3 text-sm text-gray-500"
          >
            {{ reaction.username }}
          ...
          <div v-else class="flex w-full items-center justify-between gap-3 text-sm text-gray-500">
            You
