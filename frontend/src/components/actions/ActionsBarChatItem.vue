<script setup lang="ts">
  ...
  const sendMessage = async () => {
    ...
    try {
      chatStore.sendMessage(
        ...
        replyMesssage.value ? replyMesssage.value._id : null,
        fileUrls
      );
  ...
  watch(
    () => actionsStore.chatActionsVisible,
    newVal => {
      if (newVal) {
        reset();
      }
...
<template>
  <div>
    <Drawer v-model:visible="actionsStore.chatActionsVisible" position="right" :style="{ width: 500 + 'px' }">
      <template #header>
        <div class="flex w-full items-center">
          <h3 class="text-xl font-semibold">
            {{ t('chat.chat') }} <span class="text-blue-500">{{ chatStore.messagesListLength }}</span>
            {{ t('chat.messages') }}
          </h3>
          <Button
            icon="pi pi-sync"
      ...
      <div class="flex h-[90vh] flex-col justify-between gap-0" ref="messageEmojiPicker">
        ...
        <!-- messages -->
        <div ref="messagesContainer" class="card mb-3 flex-1 overflow-y-auto rounded-md border border-gray-300 p-3">
          <template v-for="message in chatStore.messagesList" :key="message._id">
            ...
            >
              <Button
                ...
                severity="secondary"
                variant="text"
                class="absolute bottom-[-10px] left-[10px] m-0 p-0"
              ...
              <div v-if="message.replyTo" class="flex w-full bg-amber-50 p-1 px-2">
                ...
                <button class="cursor-pointer" @click="scrollToMessage(message.replyTo._id)">
        ...
        <div v-if="previewUrls.length" class="mb-2 flex flex-wrap gap-2">
          <div v-for="(url, i) in previewUrls" :key="i" class="relative">
            <img v-if="url.startsWith('data:image/')" :src="url" class="h-20 w-20 rounded-md object-cover" />
            <div v-else class="flex h-20 w-20 items-center justify-center overflow-hidden rounded-md border">
              <div>
                {{ url }}
              </div>
            </div>
